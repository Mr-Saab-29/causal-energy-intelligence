...
def summarize_ranking_shifts(
    ...
    return {
        ...
        "aggregate": sanitize_json_value(summarize_shift_aggregate(aggregate)),
...
def run_causal_adjusted_recommendations(
    ...
    if ensure_marginal_proxy and not Path(marginal_proxy_path).exists():
    ...
    marginal_proxy = pd.read_csv(marginal_proxy_path, parse_dates=[TIMESTAMP_COLUMN])
    ...
    metrics["generated_from"] = {
        ...
        "marginal_proxy": str(marginal_proxy_path),
    }
    metrics["outputs"] = {
