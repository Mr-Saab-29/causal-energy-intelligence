...
def test_causal_adjusted_scenario_recommendations_change_by_scenario() -> None:
    average_rankings = sample_average_rankings()
    average_rankings["predicted_avg_price_eur_mwh"] = [10.0, 1.0, 9.0]
    average_rankings["actual_avg_price_eur_mwh"] = [10.0, 1.0, 9.0]
    average_rankings["predicted_price_rank"] = [3, 1, 2]
    average_rankings["actual_price_rank"] = [3, 1, 2]
    average_rankings["predicted_price_rank_pct"] = [1.0, 0.0, 0.5]
    average_rankings["actual_price_rank_pct"] = [1.0, 0.0, 0.5]
    marginal_rankings = build_marginal_workload_rankings(
        average_rankings,
        sample_marginal_proxy(),
    )

    recommendations, metrics = build_causal_adjusted_scenario_recommendations(
        marginal_rankings,
        top_n=2,
    )

    first_by_scenario = (
        recommendations[recommendations["recommendation_rank"] == 1]
        .set_index("scenario")["timestamp_utc"]
        .to_dict()
    )
    assert first_by_scenario["emissions_reduction"] == pd.Timestamp("2026-01-01T02:00:00Z")
    assert first_by_scenario["budget_control"] == pd.Timestamp("2026-01-01T01:00:00Z")
    assert recommendations["recommendation_basis"].eq("causal_adjusted_mvp").all()
    assert "predicted_marginal_carbon_intensity_g_co2e_per_kwh" in recommendations
    assert metrics["scenarios"][0]["scenario"] == "emissions_reduction"
...
def test_run_causal_adjusted_recommendations_writes_outputs(tmp_path) -> None:
    ...
    scenario_recommendation_path = tmp_path / "scenario_recommendations.csv"
    ...
    result = run_causal_adjusted_recommendations(
        ...
        scenario_recommendation_output_path=scenario_recommendation_path,
    ...
    assert len(pd.read_csv(scenario_recommendation_path)) == 6
...
def test_run_all_causal_adjusted_recommendations_writes_future_outputs(tmp_path) -> None:
    ...
    future_scenario_recommendation_path = tmp_path / "future_scenario_recommendations.csv"
    metrics_path = tmp_path / "metrics.json"
    ...
    with (
        ...
        patch.object(
            ...
            str(tmp_path / "historical_scenario_recommendations.csv"),
        ),
        patch.object(
            causal_recommendations,
        ...
        patch.object(
            ...
            str(future_scenario_recommendation_path),
        ),
    ...
    assert future_scenario_recommendation_path.exists()
