...
- Causal-adjusted MVP recommendations are written to `reports/recommendations/future_causal_adjusted_workload_recommendations.csv` using a marginal-emissions proxy and explicit average-carbon fallback labels.
...
- Scenario reranking is available for clean-first, balanced, and cost-aware-clean preferences. The dashboard scenario selector changes the active future top-5 recommendation list, KPIs, and chart. The dashboard also has a recommendation-basis selector for the causal-adjusted MVP path when marginal proxy artifacts are available.
...
- `make causal-recommendations` builds historical and future marginal-proxy rankings, causal-adjusted top-5 recommendations, and ranking-shift metrics.
...
- Future marginal-proxy decision rankings: `reports/rankings/future_marginal_workload_decision_rankings.csv`
